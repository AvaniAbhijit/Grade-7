...
# Load sounds
egg_collect_sound = pygame.mixer.Sound("collect.wav")
game_over_sound = pygame.mixer.Sound("gameover.wav")

# Level initialization
level = 1
level_threshold = 10  # Increase the difficulty every 10 points
...
while True:
    ...
    for egg_x, egg_y in eggs:
        ...
        if basket_x < egg_x < basket_x + basket_width and basket_y < updated_y < basket_y + basket_height:
            ...
            egg_collect_sound.play()  # Play sound effect for egg collection
            
            # Check if the score has reached the level threshold
            if score % level_threshold == 0:
                level += 1
                egg_speed += 1  # Increase egg fall speed with each level
    ...
    # Display score, missed eggs count, and level
    score_surface = font.render(f"Score: {score}", True, (0, 0, 0))
    missed_surface = font.render(f"Missed: {missed_eggs}", True, (255, 0, 0))
    level_surface = font.render(f"Level: {level}", True, (0, 0, 255))
    screen.blit(score_surface, (10, 10))
    screen.blit(missed_surface, (10, 50))
    screen.blit(level_surface, (10, 90))
...
# Wait before quitting to allow the game over sound to finish
